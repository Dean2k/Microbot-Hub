package net.runelite.client.plugins.microbot.shrekfighter.bank;
...
import net.runelite.client.plugins.microbot.shrekfighter.shop.ShopItem;
import net.runelite.client.plugins.microbot.shrekfighter.shop.ShopScript;
import net.runelite.client.plugins.microbot.shrekfighter.shop.ShopType;
...
public class BankerScript extends Script {
    ...
    public boolean run(ShrekFighterConfig config) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if (config.bank() && needBanking() && !ShrekFighterPlugin.needShopping) {
                    if(handleBanking()){
    ...
    public boolean needBanking() {
        if(config.currentInventorySetup() == null){
            if(config.defaultInventorySetup() != null) {
                ShrekFighterPlugin.setCurrentSlayerInventorySetup(config.defaultInventorySetup());
            }
        }        
        if(!config.bank()){
            return false;
    ...
    public void withdrawUpkeepItems(ShrekFighterConfig config) {
        ...
        if (config.useInventorySetup() || config.slayerMode()) {
            ...
            // Deposit all inventory items first so all loot is banked
            Rs2Bank.depositAll();

            inventorySetup.loadEquipment();
            inventorySetup.loadInventory();

            // Withdraw extra items specified in config (e.g. brimstone key, dusty key)
            withdrawExtraItems(config);
                    ...
                    if (Rs2Bank.hasBankItem(config.slayerTaskWeaknessItem())) {
                        ...
                        if(Rs2Bank.hasBankItem(config.slayerTaskWeaknessItem(), item.isStackable() ? Rs2Slayer.getSlayerTaskSize() : 1,false)) {
                            ...
                            ShopScript.shopItems.add(new ShopItem(config.slayerTaskWeaknessItem(), itemComp.isStackable() ? Rs2Slayer.getSlayerTaskSize() : 1, itemComp.getId() , isGeItem ? ShopType.GRAND_EXCHANGE : ShopType.SLAYER_SHOP));
                             ShrekFighterPlugin.needShopping = true;
                        }
                ...
                if (needProtectiveSlayerEquipment()) {
                    if (Rs2Bank.hasBankItem(Rs2Slayer.getSlayerTaskProtectiveEquipment())) {
                        ...
                        InventorySetup inventorySetup1 = MInventorySetupsPlugin.getInventorySetups().stream().filter(Objects::nonNull).filter(x -> x.getName().equalsIgnoreCase("cache")).findFirst().orElse(null);
                         ShrekFighterPlugin.setCurrentSlayerInventorySetup(inventorySetup1);
                    } else {
            ...
            // No inventory setup — just deposit everything and withdraw extra items
            if (!Rs2Bank.isOpen()) {
                Microbot.log("Bank didn't open, returning.");
                return;
            }
    ...
    private void withdrawExtraItems(ShrekFighterConfig config) {
        String extraItems = config.extraItemsToWithdraw();
        if (extraItems == null || extraItems.trim().isEmpty()) return;

        String[] items = extraItems.split(",");
        for (String entry : items) {
            ...
            if (entry.contains(":")) {
                ...
                try { quantity = Integer.parseInt(parts[1].trim()); } catch (NumberFormatException e) { quantity = 1; }
            } else {
                itemName = entry;
            }
            ...
            if (bankItem != null) {
                ...
                Microbot.log("Extra item not found in bank: " + itemName);
            }
        }
    }

    public boolean handleBanking() {
         ShrekFighterPlugin.setState(State.BANKING);
        //bankingTriggered = true;
