package net.runelite.client.plugins.microbot.shrekfighter.model;
...
public class InventorySetupUtil {
    public static ShrekFighterConfig config;
    
    @Inject
    public InventorySetupUtil(ShrekFighterConfig config) {
    ...
    public static void determineInventorySetup(String monsterName) {
        ...
        if (!Collections.disjoint(requiredSet, dragonWyvernItems)) {
            ...
            ShrekFighterPlugin.setCurrentSlayerInventorySetup(config.dragonWyvernInventorySetup());
            return;
        }
        if (!Collections.disjoint(requiredSet, drakeItems)) {
            ...
            ShrekFighterPlugin.setCurrentSlayerInventorySetup(config.drakeInventorySetup());
            return;
        }
        if (!Collections.disjoint(requiredSet, kuraskTurothItems)) {
            ...
            ShrekFighterPlugin.setCurrentSlayerInventorySetup(config.kuraskTurothInventorySetup());
            return;
        }
        if (!Collections.disjoint(requiredSet, caveMiscItems)) {
            ...
            ShrekFighterPlugin.setCurrentSlayerInventorySetup(config.caveMiscellaneousInventorySetup());
            return;
        }
