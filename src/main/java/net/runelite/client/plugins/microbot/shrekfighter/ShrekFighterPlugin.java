...
import net.runelite.client.plugins.microbot.shrekfighter.bank.BankerScript;
import net.runelite.client.plugins.microbot.shrekfighter.combat.*;
...
import net.runelite.client.plugins.microbot.shrekfighter.loot.LootScript;
import net.runelite.client.plugins.microbot.shrekfighter.safety.SafetyScript;
import net.runelite.client.plugins.microbot.shrekfighter.shop.ShopScript;
import net.runelite.client.plugins.microbot.shrekfighter.skill.AttackStyleScript;
...
@PluginDescriptor(
        name = PluginConstants.SHREK + "Shrek Fighter",
        authors = { "Mocrosoft", "See1Duck", "ShrekamusChrist" },
        version = ShrekFighterPlugin.version,
        minClientVersion = "2.1.32",
        description = "Microbot Shrek Fighter plugin",
        tags = {"fight", "microbot", "misc", "combat", "playerassistant"},
        cardUrl = "https://chsami.github.io/Microbot-Hub/ShrekFighterPlugin/assets/card.png",
        iconUrl = "https://chsami.github.io/Microbot-Hub/ShrekFighterPlugin/assets/icon.png",
        enabledByDefault = PluginConstants.DEFAULT_ENABLED,
        isExternal = PluginConstants.IS_EXTERNAL
)
@Slf4j
public class ShrekFighterPlugin extends Plugin {
    ...
    @Inject
    private ShrekFighterConfig config;
    @Inject
    private OverlayManager overlayManager;
    @Inject
    private ShrekFighterOverlay playerAssistOverlay;
    @Inject
    private ShrekFighterInfoOverlay playerAssistInfoOverlay;
    private Point lastMenuOpenedPoint;
    private ShrekFighterPanel npcDialog;
    private WorldPoint trueTile;

    @Provides
    public ShrekFighterConfig provideConfig(ConfigManager configManager) {
        return configManager.getConfig(ShrekFighterConfig.class);
    }
    ...
    private void showNpcSearchDialog() {
        if (npcDialog != null) {
            npcDialog.dispose();
        }
        npcDialog = new ShrekFighterPanel(null);
    ...
    private static <T> void setConfig(String key, T value) {
        Microbot.getConfigManager().setConfiguration(ShrekFighterConfig.GROUP, key, value);
    }

    private static <T> T getConfig(String key, Class<T> type) {
        return Microbot.getConfigManager().getConfiguration(ShrekFighterConfig.GROUP, key, type);
    }
    ...
    public static String getNpcAttackList() {
        return Microbot.getConfigManager().getConfiguration(ShrekFighterConfig.GROUP, "monster");
    }
    ...
    public void onConfigChanged(ConfigChanged event) {
        ...
        if (ShrekFighterConfig.GROUP.equals(event.getGroup()) && event.getKey().equals(HIGH_ALCH_BLACKLIST_KEY)) {
        ...
        if (ShrekFighterConfig.GROUP.equals(event.getGroup()) && "searchNpcs".equals(event.getKey())) {
            showNpcSearchDialog();
            return;
        }
    ...
    public void onGameTick(GameTick gameTick) {
        try {
            ...
            log.info("Shrek Fighter PluginonGameTick Error: " + e.getMessage());
        }
    }
    ...
    public void onNpcDespawned(NpcDespawned npcDespawned) {
        try {
            ...
            log.info("Shrek Fighter PluginonNpcDespawned Error: " + e.getMessage());
        }
    }
    ...
    public void onHitsplatApplied(HitsplatApplied event){
        try {
            ...
            log.info("Shrek Fighter PluginonHitsplatApplied Error: " + e.getMessage());
        }
    }
    @Subscribe
